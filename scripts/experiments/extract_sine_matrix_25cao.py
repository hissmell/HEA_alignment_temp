"""
Extract Sine Matrix representations for 25Cao dataset
...
def extract_sine_matrices(tasknames, adsorbate="O", chunk_size=1000):
    """Extract Sine Matrix representations in chunks
    ...
    # Setup extractor - Sine Matrix is optimized for periodic systems
    extractor = SineMatrixExtractor(
    ...
    output_dir = Path("/DATA/user_scratch/pn50212/2024/12_AtomAttention/datasets/25Cao/representations/sine_matrix")
    ...
    if start_chunk == 0:
        ...
        # Extract the actual representation array from the result dict - looking for sm_all key
        first_repr = first_result.get('sm_all', None)
        ...
        print(f"First structure PBC: {pbc_info}")
    ...
    for chunk_idx in range(start_chunk, total_chunks):
        ...
        for taskname in tqdm(chunk_tasknames, desc=f"Chunk {chunk_idx}"):
            try:
                ...
                # Check if structure is periodic (important for Sine Matrix)
                if not any(atoms.pbc):
                    non_periodic_count += 1

                # Extract representation
                result = extractor.extract_single(atoms)
                # Get the actual representation array from the result dict - looking for sm_all key
                representation = result.get('sm_all', None)

                if representation is not None:
                    # Handle sorted_l2 auto-flattening issue (same as Coulomb Matrix)
        ...
        if non_periodic_count > 0:
            print(f"Warning: {non_periodic_count} structures in this chunk have no periodic boundaries")
    ...
    if all_structure_info:
        ...
        column_order = ['structure_id', 'taskname', 'adsorbate', 'file_path', 'chunk_file',
                       ...
                       'extraction_success', 'permutation', 'n_atoms_max', 'flatten', 'dtype']
        ...
        if 'has_periodicity' in df.columns:
            ...
            print(f"Periodic structures: {periodic_count}/{total_count}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract Sine Matrix for 25Cao dataset")
    ...
    # Start extraction
    print("\n" + "="*50)
    print(f"Starting Sine Matrix extraction for {args.adsorbate}")
    print("="*50)

    extract_sine_matrices(tasknames, adsorbate=args.adsorbate, chunk_size=args.chunk_size)