"""
Extract Ewald Sum Matrix representations for 25Cao dataset
Saves in chunked JSON format (1000 structures per file)
Note: Ewald Sum Matrix is designed for periodic structures only
"""
...
def extract_ewald_sum_matrices(tasknames, adsorbate="O", chunk_size=1000):
    """Extract Ewald Sum Matrix representations in chunks
    ...
    # Setup extractor - Ewald Sum Matrix is for periodic systems only
    extractor = EwaldSumMatrixExtractor(
    ...
    # Set default Ewald parameters for better convergence
    extractor.set_ewald_parameters(
        accuracy=1e-5,  # Convergence accuracy
        weight=1.0      # Balance between real/reciprocal space
    )

    # Output directory - unified for all adsorbates
    output_dir = Path("/DATA/user_scratch/pn50212/2024/12_AtomAttention/datasets/25Cao/representations/ewald_sum_matrix")
    ...
    if start_chunk == 0:
        ...
        # Check if structure is periodic (REQUIRED for Ewald Sum Matrix)
        if not any(first_atoms.pbc):
            print("ERROR: First structure is non-periodic. Ewald Sum Matrix requires periodic structures!")
            print(f"PBC: {first_atoms.pbc}")
            return

        first_result = extractor.extract_single(first_atoms)

        # Extract the actual representation array from the result dict - looking for esm_all key
        first_repr = first_result.get('esm_all', None)
        ...
        # Check if structures have periodic boundary conditions
        pbc_info = first_atoms.pbc
        print(f"First structure PBC: {pbc_info} (All should be True for ESM)")
        print(f"First structure cell det: {np.linalg.det(first_atoms.cell.array):.4f}")

        metadata = {
            "descriptor": "ewald_sum_matrix",
            "adsorbate": adsorbate,
            "parameters": {
    ...
    for chunk_idx in range(start_chunk, total_chunks):
        ...
        for taskname in tqdm(chunk_tasknames, desc=f"Chunk {chunk_idx}"):
            try:
                ...
                if not any(atoms.pbc):
                    ...
                    print(f"ERROR: Non-periodic structure {taskname}, skipping...")
                    # Track failed extraction due to non-periodicity
                    structure_info = {
                        ...
                        'error': 'Non-periodic structure'
                    }
                    all_structure_info.append(structure_info)
                    continue

                # Check for valid unit cell
                cell_det = np.linalg.det(atoms.cell.array)
                if cell_det == 0:
                    invalid_cell_count += 1
                    print(f"ERROR: Invalid unit cell for {taskname}, determinant=0")
                    continue

                # Extract representation
                result = extractor.extract_single(atoms)
                # Get the actual representation array from the result dict - looking for esm_all key
                representation = result.get('esm_all', None)

                if representation is not None:
                    # Handle sorted_l2 auto-flattening issue (same as Coulomb/Sine Matrix)
                    if extractor.permutation == 'sorted_l2' and representation.ndim == 1:
                        matrix_size = int(np.sqrt(len(representation)))
                        representation = representation.reshape(matrix_size, matrix_size)
                    ...
                    structure_info = {
                        ...
                        'pbc': str(atoms.pbc),  # Periodic boundary conditions
                        'has_periodicity': all(atoms.pbc),  # All True for ESM
                        'cell_det': float(cell_det),  # Unit cell determinant
                        'permutation': extractor.permutation,
                        'n_atoms_max': extractor.n_atoms_max,
                        'flatten': extractor.flatten,
                        'dtype': extractor.dtype,
                    ...
                    structure_info = {
                        ...
                        'matrix_dim': None,
                        'pbc': str(atoms.pbc),
                        'has_periodicity': all(atoms.pbc),
                        'cell_det': float(cell_det),
                        'permutation': extractor.permutation,
                        'n_atoms_max': extractor.n_atoms_max,
                        'flatten': extractor.flatten,
                        'dtype': extractor.dtype,
        ...
        if non_periodic_count > 0:
            print(f"ERROR: {non_periodic_count} non-periodic structures in chunk (ESM requires periodic!)")

        if invalid_cell_count > 0:
            print(f"ERROR: {invalid_cell_count} structures with invalid unit cells")
    ...
    if all_structure_info:
        ...
        column_order = ['structure_id', 'taskname', 'adsorbate', 'file_path', 'chunk_file',
                       'n_atoms', 'representation_shape', 'matrix_dim', 'pbc', 'has_periodicity',
                       'cell_det', 'extraction_success', 'permutation', 'n_atoms_max', 'flatten',
                       'dtype', 'accuracy', 'weight']
        ...
        if 'has_periodicity' in df.columns:
            periodic_count = df['has_periodicity'].sum()
            print(f"Fully periodic structures: {periodic_count}/{total_count}")
            if periodic_count < total_count:
                print(f"WARNING: {total_count - periodic_count} structures are not fully periodic!")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract Ewald Sum Matrix for 25Cao dataset")
    ...
    # Start extraction
    print("\n" + "="*50)
    print(f"Starting Ewald Sum Matrix extraction for {args.adsorbate}")
    print("NOTE: Ewald Sum Matrix requires periodic structures!")
    print("="*50)

    extract_ewald_sum_matrices(tasknames, adsorbate=args.adsorbate, chunk_size=args.chunk_size)