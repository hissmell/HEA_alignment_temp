#!/usr/bin/env python3
"""
Orb baseline performance on 25Cao dataset
Conda env: orb
"""
...
# orb model name → pretrained function name
ORB_MODELS = {
...
class OrbPredictor:
    ...
    def load_model(self):
        ...
        fn_name = ORB_MODELS.get(self.model_name)
        if fn_name is None:
            raise ValueError(f"Unknown Orb model: {self.model_name}. "
                             f"Available: {list(ORB_MODELS)}")

        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        orbff = getattr(pretrained, fn_name)(device=device, precision="float32-high")
        self.calculator = ORBCalculator(orbff, device=device)
        print(f"Loaded Orb model: {self.model_name} on {device}")
...
def main():
    ...
    parser.add_argument('--models', nargs='+', default=list(ORB_MODELS.keys()))
    ...
    for model_name in args.models:
        ...
        predictor = OrbPredictor(model_name)
    ...
    excel_path = OUTPUT_DIR / 'baseline_orb.xlsx'
